...
```bash
flutter pub add drift:^2.20.0 drift_flutter:^0.2.0 flutter_riverpod:^2.6.1 pdf:^3.11.0 printing:^5.14.0 intl:^0.20.2
flutter pub add flutter_localizations --sdk=flutter
flutter pub add dev:drift_dev:^2.20.0 dev:build_runner:^2.4.6 dev:sqlite3:^2.4.6
```

Três restrições de resolução que só aparecem quando você tenta: `printing` da linha
5.14 é a que casa com `pdf` 3.11; `intl` tem que ser 0.20.2 porque é o que o
`flutter_localizations` deste SDK exige; e `build_runner` abaixo de 2.4.6 e `sqlite3`
abaixo de 2.4.6 são velhos demais para `drift_dev` 2.20 e para `printing`,
respectivamente.

`flutter pub add` grava versão exata, sem caret. Depois de adicionar, edite o
`pubspec.yaml` para deixar `^` em todas elas e rode `flutter pub get` — pin exato
bloqueia até correção de bug em patch.
