...
void main() {
  ...
  /// Espera o provider sair de loading e devolve o relatório.
  Future<ReportData> lerRelatorio(ProviderContainer c, int loteId) async {
    final sub = c.listen(relatorioProvider(loteId), (_, __) {});
    while (sub.read() is! AsyncData<ReportData>) {
  ...
  test('editar o peso atualiza o total sem invalidacao manual', () async {
    ...
    r = await lerRelatorio(container, id);
  ...
  test('adicionar regra de raca muda o valor do animal que casa', () async {
    final id = await loteComUmAnimal();
    final animalId = (await lerRelatorio(container, id)).linhas.single.animalId;
...
void main() {
  ...
  Future<int> criarLote() => LoteRepository(db).criar(
        nome: 'Boiada Janeiro',
        data: DateTime(2026, 7, 31),
        criterio: CriterioBase.arroba,
        precoBaseCentavos: 32000,
  ...
  Future<void> abrir(WidgetTester tester, int loteId) async {
    await tester.pumpWidget(ProviderScope(
      overrides: [dbProvider.overrideWithValue(db)],
      child: MaterialApp(home: PesagemScreen(loteId: loteId)),
    ));
    await tester.pumpAndSettle();
  }

  testWidgets('digitar 480 e salvar soma o animal ao total', (tester) async {
    final loteId = await criarLote();
    await abrir(tester, loteId);
    ...
    // A prévia aparece antes de salvar.
    expect(find.text(r'16,64 @   R$ 5.324,80'), findsOneWidget);

    await tester.tap(find.text('SALVAR'));
    await tester.pumpAndSettle();

    expect(find.text(r'R$ 5.324,80'), findsOneWidget);
    expect(find.text('1'), findsWidgets, reason: 'contador de cabecas');
  });
  ...
  testWidgets('desfazer remove o ultimo animal', (tester) async {
    final loteId = await criarLote();
    await abrir(tester, loteId);
...
Se a busca por texto falhar por causa de espaçamento, ajuste a expectativa ao texto que a tela realmente produz — mas não mude a tela só para o teste passar sem entender o motivo.
...
Run: `flutter test`
Expected: todos passando. Contagem esperada: 4 (units) + 11 (parse) + 7 (format) + 13 (pricing) + 4 (report_data) + 3 (database) + 7 (repositories) + 4 (providers) + 3 (pesagem) + 5 (pdf) = 61 testes.
